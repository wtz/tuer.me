...
exports.getAvatar = function(uid, width, height, callback) {
  tuerBase.findUser(uid, function(err, user) {
    if (err) callback(err);
      ...
      img.onload = function() {
        var canvas, ctx;
        if (user.coords && user.coords !== 'undefined' && width && height) {
          ...
          ctx = canvas.getContext('2d');
          if (width && height) {
            var sy = Math.min(img.width, img.height);
            ctx.drawImage(img, 0, 0, sy, sy, 0, 0, w, h);
          } else {
...
exports.getUrl = function(pid) {
  return 'http://' + config.imagehost+ '/avatar/small/' + pid + '.png';
};

exports.getArtUrl = function(pid) {
  return 'http://' + config.imagehost+ '/avatar/big/' + pid + '.png';
};
