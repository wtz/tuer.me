...
myOAP.on('enforce_login', function(req, res, authorize_url, next) {
	if (req.session.is_login) {
		...
		var redirect_url = '/login?next=' + encodeURIComponent('http://www.tuer.me' + authorize_url);
		if (req.query.type) redirect_url += '&type=' + req.query.type;
		//如果带了x_renew=true,则退出之后再跳转，实现切换账户或者退出功能
		//退出时，退出app，不退出账户。切换账户时，关闭app，其实就算是退出了。
		//如果需要注销功能，则访问这个地址之后重启app?
		if (req.query.x_renew && req.session.is_login) {
...
myOAP.on('authorize_form', function(req, res, client_id, authorize_url) {
	...
	'apis', function(err, api) {
		...
		if (api && redirect_url != api.appcallback) {
			error = 'redirect_uri 不是预留的url';
		}
		if (type && type == 'h5') {
			template = 'custom/authorize_form_h5';
		} else {
			template = 'custom/authorize_form';
		}
		res.render(template, {
			config: config,
